...
typedef int32_t (*ST25DVxxKC_Write_Func)(const struct device *dev, const uint16_t, const uint16_t,
					 const uint8_t *const, const uint16_t);
typedef int32_t (*ST25DVxxKC_Read_Func)(const struct device *dev, const uint16_t, const uint16_t,
					uint8_t *const, const uint16_t);
typedef int32_t (*ST25DVxxKC_IsReady_Func)(const struct device *dev, const uint16_t,
					   const uint32_t);
...
typedef struct {
	...
	int32_t (*ReadData)(const struct device *, const ST25DVxxKC_Object_t *const, uint8_t *const,
			    const uint16_t, const uint16_t);
	int32_t (*WriteData)(const struct device *, const ST25DVxxKC_Object_t *const,
			     const uint8_t *const, const uint16_t, const uint16_t);
} ST25DVxxKC_Drv_t;
...
/** @brief I2C Time out (ms), max value : tw + tw * (Max write bytes) / (Internal page write) :  5 +
 * 5*(256/16). */
...
/* Exported functions ------------------------------------------------------- */
extern int32_t ST25DVxxKC_ReadRegister(const struct device *dev,
				       const ST25DVxxKC_Object_t *const pObj, uint8_t *const pData,
				       const uint16_t TarAddr, const uint16_t NbByte);
extern int32_t ST25DVxxKC_WriteRegister(const struct device *dev, ST25DVxxKC_Object_t *const pObj,
...
int32_t ST25DVxxKC_ResetRFSleep_Dyn(const struct device *dev,
