 ...
 * @details  As recommended by the STM32 Cube methodology, this driver provides a standard structure
 * to expose the NFC tag standard API.\n It also provides an extended API through its extended
 * driver structure.\n To be usable on any MCU, this driver calls several IOBus functions. The IOBus
 * functions are implemented outside this driver, and are in charge of accessing the MCU peripherals
 * used for the communication with the tag.
 * @{
 */
...
static int32_t ReadRegWrap(const struct device *dev, const void *const Handle, const uint16_t Reg,
			   uint8_t *const pData, const uint16_t Length);
static int32_t WriteRegWrap(const struct device *dev, const void *Handle, const uint16_t Reg,
			    const uint8_t *const pData, const uint16_t Length);

int32_t ST25DVxxKC_Init(const struct device *dev, ST25DVxxKC_Object_t *const pObj);
int32_t ST25DVxxKC_ReadID(const struct device *dev, const ST25DVxxKC_Object_t *const pObj,
			  uint8_t *const pICRef);
int32_t ST25DVxxKC_IsDeviceReady(const struct device *dev, const ST25DVxxKC_Object_t *const pObj,
				 const uint32_t Trials);
int32_t ST25DVxxKC_ReadData(const struct device *dev, const ST25DVxxKC_Object_t *const pObj,
			    uint8_t *const pData, const uint16_t TarAddr, const uint16_t NbByte);
int32_t ST25DVxxKC_WriteData(const struct device *dev, const ST25DVxxKC_Object_t *const pObj,
			     const uint8_t *const pData, const uint16_t TarAddr,
 ...
 * @details  Provides a generic way to access the ST25DVxxKC implementation of the NFC tag standard
 * driver functions.
 */
ST25DVxxKC_Drv_t St25Dvxxkc_Drv = {ST25DVxxKC_Init, ST25DVxxKC_ReadID, ST25DVxxKC_IsDeviceReady,
				   ST25DVxxKC_ReadData, ST25DVxxKC_WriteData};
 ...
 */
int32_t ST25DVxxKC_RegisterBusIO(const struct device *dev, ST25DVxxKC_Object_t *const pObj,
			     ...
			     const uint16_t NbByte)
{
	...
	do {
		...
		ret = pObj->IO.Write(dev, pObj->IO.DeviceAddress, mem_addr, pdata_index,
				     split_data_nb);
		if (ret == NFCTAG_OK) {
 ...
 * @return int32_t enum status.
 */
int32_t ST25DVxxKC_ReadRegister(const struct device *dev, const ST25DVxxKC_Object_t *const pObj,
				uint8_t *pData, const uint16_t TarAddr, const uint16_t NbByte)
{
	/* Read Data in system memory */
	return pObj->IO.Read(dev,
			     pObj->IO.DeviceAddress | ST25DVXXKC_ADDR_SYSTEMMEMORY_BIT_I2C |
				     ST25DVXXKC_ADDR_MODE_BIT_I2C,
			     TarAddr, pData, NbByte);
}
 ...
 * @return   int32_t enum status.
 */
int32_t ST25DVxxKC_WriteRegister(const struct device *dev, ST25DVxxKC_Object_t *const pObj,
				 const uint8_t *const pData, const uint16_t TarAddr,
				 const uint16_t NbByte)
{
	int32_t ret;
	...
	do {
		...
		ret = pObj->IO.Write(dev,
				     pObj->IO.DeviceAddress | ST25DVXXKC_ADDR_SYSTEMMEMORY_BIT_I2C |
					     ST25DVXXKC_ADDR_MODE_BIT_I2C,
				     mem_addr, pdata_index, split_data_nb);
		if (ret == NFCTAG_OK) {
			int32_t pollstatus;
			/* Poll until EEPROM is available */
			uint32_t tickstart = pObj->IO.GetTick();

			/* Special case for ST25DVXXKC_I2CCFG_REG: align IO.DeviceAddress with */
			/* register content */
			if ((mem_addr <= ST25DVXXKC_I2CCFG_REG) &&
			    (mem_addr + split_data_nb >= ST25DVXXKC_I2CCFG_REG)) {
				uint8_t deviceCode =
					(pdata_index[ST25DVXXKC_I2CCFG_REG - mem_addr] &
					 ST25DVXXKC_I2CCFG_DEVICECODE_MASK) >>
					ST25DVXXKC_I2CCFG_DEVICECODE_SHIFT;
				uint8_t E0 = (pdata_index[ST25DVXXKC_I2CCFG_REG - mem_addr] &
					      ST25DVXXKC_I2CCFG_E0_MASK) >>
					     ST25DVXXKC_I2CCFG_E0_SHIFT;

				pObj->IO.DeviceAddress =
					(pObj->IO.DeviceAddress &
					 ~(ST25DVXXKC_ADDR_DEVICECODE_MASK)) |
					((deviceCode << ST25DVXXKC_ADDR_DEVICECODE_SHIFT) &
					 ST25DVXXKC_ADDR_DEVICECODE_MASK);
				pObj->IO.DeviceAddress =
					(pObj->IO.DeviceAddress & ~(ST25DVXXKC_ADDR_E0_MASK)) |
					((E0 << ST25DVXXKC_ADDR_E0_SHIFT) &
					 ST25DVXXKC_ADDR_E0_MASK);
			}

			/* Wait until ST25DVxxKC is ready or timeout occurs */
			do {
				pollstatus = pObj->IO.IsReady(dev, pObj->IO.DeviceAddress, 1);
			} while (((uint32_t)pObj->IO.GetTick() <
				  tickstart + ST25DVXXKC_WRITE_TIMEOUT) &&
 ...
 * @param[in] pObj pointer to the device structure object.
 * @param[out] pSizeInfo Pointer on a ST25DVxxKC_MEM_SIZE structure used to return the Memory size
 * information.
 * @return int32_t enum status.
 */
int32_t ST25DVxxKC_ReadMemSize(const struct device *dev, const ST25DVxxKC_Object_t *const pObj,
 ...
 * @return int32_t enum status.
 */
int32_t ST25DVxxKC_GetRFSleep_Dyn(const struct device *dev, const ST25DVxxKC_Object_t *const pObj,
 ...
 */
static int32_t ReadRegWrap(const struct device *dev, const void *const handle, const uint16_t Reg,
			   uint8_t *const pData, const uint16_t len)
{
	int32_t ret;
 ...
 * @return int32_t enum status.
 */
static int32_t WriteRegWrap(const struct device *dev, const void *const handle, const uint16_t Reg,
			    const uint8_t *const pData, const uint16_t len)
{
	int32_t ret;
	ST25DVxxKC_Object_t *pObj = (ST25DVxxKC_Object_t *)handle;
	...
	if ((Reg & (ST25DVXXKC_IS_DYNAMIC_REGISTER)) == ST25DVXXKC_IS_DYNAMIC_REGISTER) {
		...
		if (ret == NFCTAG_OK) {
			...
			if (Reg == ST25DVXXKC_I2CCFG_REG) {
				uint8_t deviceCode = (*pData & ST25DVXXKC_I2CCFG_DEVICECODE_MASK) >>
						     ST25DVXXKC_I2CCFG_DEVICECODE_SHIFT;
				uint8_t E0 = (*pData & ST25DVXXKC_I2CCFG_E0_MASK) >>
					     ST25DVXXKC_I2CCFG_E0_SHIFT;
