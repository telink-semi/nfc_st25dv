 ...
 *  @{
 *  @brief    This module provides an abstraction layer to the NDEF library for the NFC Forum Type5
 Tag.
 ...
 *
 |:------------:|:---------------------------:|:---:|:--------------:|:---:|:---:|:---------:|:---------:|
 *             | Magic Number | Version & Access Conditions | 00h | NFC Type 5 Tag |RFU  |RFU
...
uint16_t NfcType5_ReadNDEF(const struct device *dev, uint8_t *pData)
{
	...
	/* Read TL of Type 5 */
	status = NDEF_Wrapper_ReadData(dev, (uint8_t *)&tlv, CCFileStruct.NDEF_offset,
				       sizeof(TT5_TLV_t));
	if (status != NDEF_OK) {
		return status;
	}
	...
	if (DataLength > 0) {
		/* Read NDEF */
		if (NDEF_Wrapper_ReadData(dev, (pData), CCFileStruct.NDEF_offset + tlv_size,
					  DataLength) != NDEF_OK) {
			return NDEF_ERROR;
		}
	}
...
uint16_t NfcType5_WriteData(const struct device *dev, uint8_t Type, uint16_t Length, uint8_t *pData)
{
	...
	if (NDEF_Wrapper_WriteData(dev, empty_ndef_tlv, CCFileStruct.NDEF_offset,
				   sizeof(empty_ndef_tlv)) != NDEF_OK) {
		return NDEF_ERROR;
	}
	...
	if (NDEF_Wrapper_WriteData(dev, &NfcT5_Terminator, offset, sizeof(NfcT5_Terminator)) !=
	    NDEF_OK) {
		return NDEF_ERROR;
	}

	/* Finally write the correct TLV to EEPROM with the actual message length */
	if (NDEF_Wrapper_WriteData(dev, (uint8_t *)&tlv, CCFileStruct.NDEF_offset, tlv_size) !=
	    NDEF_OK) {
		return NDEF_ERROR;
	}
...
/**
 * @brief  This function initializes the Capability Container and an empty NDEF message in a NFC
 * Tag.
...
uint16_t NfcType5_NDEFDetection(const struct device *dev)
{
	...
	if ((acc_buffer[0] != NFCT5_MAGICNUMBER_E1_CCFILE) &&
	    (acc_buffer[0] != NFCT5_MAGICNUMBER_E2_CCFILE)) {
	...
	while ((NDEF_Wrapper_ReadData(dev, (uint8_t *)&tlv_detect, CCFileStruct.NDEF_offset,
				      sizeof(TT5_TLV_t)) == NDEF_OK) &&
	       (CCFileStruct.NDEF_offset < memory_size)) {
		...
		/* If Proprietary NDEF jump to end of proprietary message */
			...
			if (tlv_detect.Length == NFCT5_3_BYTES_L_TLV) {
				CCFileStruct.NDEF_offset =
					CCFileStruct.NDEF_offset + tlv_detect.Length16 + 4;
				continue;
			} else {
				CCFileStruct.NDEF_offset =
					CCFileStruct.NDEF_offset + tlv_detect.Length + 2;
				continue;
			}
		}
